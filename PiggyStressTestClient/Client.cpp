...
DWORD WINAPI CClient::_WorkerThread(LPVOID lpParam)
{
	...
	TRACE("向服务器发送信息成功: %s\n", szTemp);
	pClient->ShowMessage(_T("向服务器发送信息成功: %s"), CString(szTemp).GetString());
...
bool  CClient::EstablishConnections()
{
	...
	for (int i = 0; i < m_nThreads; i++)
	{
		...
		char * tmpbuf;
		int needBytes = 0;
		needBytes = WideCharToMultiByte(CP_OEMCP, NULL, m_strMessage, -1, NULL, NULL, 0, NULL);
		if (needBytes > 0)
		{
			tmpbuf = new char[needBytes];
			WideCharToMultiByte(CP_OEMCP, NULL, m_strMessage, -1, tmpbuf, needBytes, 0, NULL);
			sprintf(m_pParamsWorker[i].szBuffer, "%d号线程 发送数据 %s", i + 1, tmpbuf);
			delete[] tmpbuf;
		}
		else
		{
			sprintf(m_pParamsWorker[i].szBuffer, "%d号线程 发送数据 %s", i + 1, "Hello!");
		}
...
bool CClient::ConnetToServer(SOCKET *pSocket, CString strServer, int nPort)
{
	...
	*pSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
	...
	// 生成地址信息
	int needBytes = WideCharToMultiByte(CP_ACP, 0, strServer.GetString(), -1, NULL, 0, NULL, NULL);
	if (needBytes > 0)
	{
		WideCharToMultiByte(CP_ACP, 0, strServer.GetString(), -1, strip, needBytes, NULL, NULL);
	}
