...
{    
	...
	pIOCPModel->_ShowMessage(CString("工作者线程启动，ID: %d."),nThreadNo);
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
	{
		...
		{  	
			...
			{  
				pIOCPModel->_ShowMessage( _T("客户端 %s:%d 断开连接."),CString(inet_ntoa(pSocketContext->m_ClientAddr.sin_addr)), ntohs(pSocketContext->m_ClientAddr.sin_port) );
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	int needBytes = WideCharToMultiByte(CP_ACP, 0, m_strIP.GetString(), -1, NULL, 0, NULL, NULL);
	if (needBytes > 0)
	{
		WideCharToMultiByte(CP_ACP, 0, m_strIP.GetString(), -1, strip, needBytes, NULL, NULL);
	...
	int errnumber = bind(m_pListenContext->m_Socket, (struct sockaddr *) &ServerAddress, sizeof(ServerAddress));

	if (errnumber != 0) 
	{
		TRACE("the bind return -1 the wsalasterror value is :%d\n", WSAGetLastError());
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	{  
		_ShowMessage(CString("创建用于Accept的Socket失败！错误代码: %d", WSAGetLastError())); 
		return false;  
	} 
	...
	{  
		...
		{  
			_ShowMessage(CString("投递 AcceptEx 请求失败，错误代码: %d", WSAGetLastError()));  
			return false;  
		}  
	} 
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	this->_ShowMessage( _T("客户端 %s:%d 连入."), CString(inet_ntoa(ClientAddr->sin_addr)), ntohs(ClientAddr->sin_port) );
	this->_ShowMessage( _T("客户额 %s:%d 信息：%s."),CString(inet_ntoa(ClientAddr->sin_addr)), ntohs(ClientAddr->sin_port),CString(pIoContext->m_wsaBuf.buf));
...
bool CIOCPModel::_DoRecv( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	this->_ShowMessage( _T("收到  %s:%d 信息：%s"),CString(inet_ntoa(ClientAddr->sin_addr)), ntohs(ClientAddr->sin_port),CString(pIoContext->m_wsaBuf.buf ));
